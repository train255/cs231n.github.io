...
Đơn vị tính toán cơ bản của não là một **neuron**. Khoảng 86 tỷ neuron có thể được tìm thấy trong hệ thống thần kinh của con người và chúng được kết nối với nhau thông qua khoảng 10^14 - 10^15 **khớp thần kinh (synapse)**. Biểu đồ dưới đây là một bản vẽ của một neuron sinh học (trái) và một mô hình toán học mô phỏng neuron sinh học (phải). Mỗi neuron nhận được tín hiệu đầu vào từ **đuôi gai (dendrite)** của nó và tạo ra các tín hiệu đầu ra dọc theo **sợi trục (axon)** (đơn) của nó. Các sợi trục cuối cùng phân nhánh ra và kết nối với đuôi gai của các nơ-ron khác thông qua các khớp thần kinh. Trong mô hình tính toán của một nơ-ron, các tín hiệu đi dọc theo các sợi trục (ví dụ \\(x_0\\) và tương tác nhiều lần (ví dụ \\(w_0 x_0\\) với đuôi gai của các nơ-ron khác dựa trên synaptic strength tại khớp thần kinh đó (ví dụ: \\(w_0\\). Ý tưởng là các synaptic strength (các trọng số \\(w\\)) có thể học và kiểm soát sức mạnh của sự ảnh hưởng (strength of influence) (và hướng của nó: hưng phấn (trọng số dương) hay kìm nén (trọng số âm)) của một nơ ron lên một nơ ron khác. Trong một mô hình cơ bản, các đuôi gai mang tín hiệu đến cơ thể tế bào (cell body) nơi chúng được kết luận hay được tính tổng (summed). Nếu phép tính tổng vượt một ngưỡng nhất định, nơ ron có thể *bắn (fire)*, gửi một cái gai (spike) dọc theo sợi trục của nó. In the computational model, we assume that the precise timings of the spikes do not matter, and that only the frequency of the firing communicates information. Based on this *rate code* interpretation, we model the *firing rate* of the neuron with an **activation function** \\(f\\), which represents the frequency of the spikes along the axon. Historically, a common choice of activation function is the **sigmoid function** \\(\sigma\\), since it takes a real-valued input (the signal strength after the sum) and squashes it to range between 0 and 1. We will see details of these activation functions later in this section.
