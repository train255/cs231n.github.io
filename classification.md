...
K-nearest neighbor yêu cầu cài đặt giá trị *k*. Nhưng giá trị nào thì cho kết quả tốt nhất? Thêm nữa chúng ta thấy rằng có nhiều hàm tính khoảng cách khác nhau mà ta sử dụng: L1 norm, L2 norm, chúng ta cũng có nhiều lựa chọn khác nữa (ví dụ như các phép tính tích vô hướng (dot product)). Những lựa chọn này được gọi là **siêu tham số (hyperparameters)** và chúng xuất hiện rất thường xuyên trong việc thiết kế các thuật toán Machine Learning để học từ dữ liệu. Nó thường không rõ ràng trong việc nên lựa chọn giá trị nào cho phù hợp.
